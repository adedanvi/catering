			...
			if(!empty($_GET)){
				...
				?>
					...
					<form action="produk.php?act=edit&&id=<?php echo $_GET['id'] ?>" method="post" enctype="multipart/form-data">
						...
						<label>Kategori jual</label><br>
						<select name="kategori_jual_id" required class="form-control"> 
						<?php
								$katjual = mysql_query("select*from kategori_jual where id='$data->kategori_jual_id'");
								$kpj = mysql_fetch_array($katjual);
							?>
								<option value="<?php echo $kpj['id']; ?>"><?php echo $kpj['katering'] ?></option>
							<?php
								$katjual = mysql_query("select*from kategori_jual");
								while($kp = mysql_fetch_array($katjual)){
							?>
								<option value="<?php echo $kp['id']; ?>"><?php echo $kp['katering'] ?></option>
								<?php } ?>
						</select><br>
						<label>Nama</label><br>
